...
export class GuestDesignationService {
  ...
  async create(dto: CreateGuestDesignationDto, user = 'system', ip = '0.0.0.0') {
    ...
    if (dto.designation_name) {
      ...
      const check = await this.db.query('SELECT 1 FROM m_guest_designation WHERE designation_id = $1', [dto.designation_id]);
      if (check.rowCount === 0) {
        await this.db.query('INSERT INTO m_guest_designation (designation_id, designation_name, inserted_by, inserted_ip) VALUES ($1,$2,$3,$4)', [dto.designation_id, null, user, ip]);
      }
    }
