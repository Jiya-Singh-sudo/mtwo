...
export function DashboardStats() {
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto p-6 space-y-6">
      ...
      {/* Page Header */}
      <div>
        <h2 className="text-2xl font-bold text-[#00247D]">Dashboard Overview</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((statItem) => {
          ...
          return (
            <div
              key={statItem.title}
              className={`bg-white border-2 ${statItem.border} rounded-sm p-6 hover:shadow-md transition-shadow`}
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="text-sm text-gray-600">{statItem.title}</p>
                  <p className="text-xs text-gray-500">{statItem.titleHi}</p>
                  <p className="text-3xl font-bold mt-2 text-[#00247D]">{statItem.value}</p>
                  <p className="text-xs text-gray-500 mt-1">{statItem.change}</p>
                </div>

                <div className={`w-12 h-12 ${statItem.color} rounded-sm flex items-center justify-center`}>
            ...
            <div key={resource.title} className="bg-white border border-gray-200 rounded-sm p-6">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-[#00247D]">{resource.title}</h3>
                    <p className="text-xs text-gray-500">{resource.titleHi}</p>
                  </div>
                </div>

                <div className="text-right">
                  <p className="text-2xl font-bold text-[#00247D]">{percentage}%</p>
      ...
      <div className="bg-orange-50 border-l-4 border-[#FF9933] p-4 rounded-sm">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className="font-semibold text-orange-900">Attention Required | ध्यान देने योग्य</h4>
