...
interface Room {
  id: string;
  roomNo: string;
  residenceType: string;    // e.g., Jam Kiran, Jam Chetak
  residenceName: string;    // e.g., Duty Room, Single Room
...
export default function Page() {
  const [rooms, setRooms] = useState<Room[]>([
    {
      id: "1",
      roomNo: "A-101",
      residenceType: "Jam Kiran",
      residenceName: "Duty Room",
      status: "Occupied",
    ...
    },
    {
      id: "2",
      roomNo: "A-102",
      residenceType: "Jam Chetak",
      residenceName: "Single Room",
      status: "Available",
      guest: "-",
      capacity: 2,
    },
    {
      id: "3",
      roomNo: "B-201",
      residenceType: "Jam Sangram",
      residenceName: "Double Room",
      status: "Reserved",
      guest: "Reserved for VIP",
      capacity: 2,
    },
  ]);
  ...
  const filteredRooms = rooms.filter((room) =>
    ...
    room.residenceName.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  function openDetailsModal(room: Room) {
    setSelectedRoom(room);
    setIsDetailsModalOpen(true);
  }

  function openStatusModal(room: Room) {
    setSelectedRoom(room);
    setNewStatus(room.status);
    setIsStatusModalOpen(true);
  }

  function openAssignRoomModal(room: Room) {
    setSelectedRoom(room);
    setAssignForm({
  ...
  function submitAssignRoom() {
    ...
    if (!assignForm.guestName || !assignForm.checkIn) {
    ...
    setRooms((prev) =>
      prev.map((r) =>
        r.id === selectedRoom.id
          ? {
              ...r,
              residenceType: assignForm.residenceType,
              residenceName: assignForm.residenceName,
        ...
        <table className="w-full">
          ...
          <tbody>
            {filteredRooms.map((room, idx) => (
              <tr key={room.id} className={idx % 2 ? "bg-gray-50" : "bg-white"}>

                <td className="px-4 py-3 border-t">{room.roomNo}</td>
                <td className="px-4 py-3 border-t">{room.residenceType}</td>
                <td className="px-4 py-3 border-t">{room.residenceName}</td>
                ...
                <td className="px-4 py-3 border-t">{room.guest}</td>
          ...
          <div className="nicModal">
            ...
            <div className="py-2">
              <div className="grid grid-cols-2 gap-4">
                ...
                <p><strong>Residence Name:</strong> {selectedRoom.residenceName}</p>
                <p><strong>Status:</strong> {selectedRoom.status}</p>
                <p><strong>Guest:</strong> {selectedRoom.guest}</p>
        ...
        <div className="modalOverlay">
          <div className="nicModal">
            ...
            <div className="nicModalActions">
              ...
              <button className="saveBtn" onClick={submitAssignRoom}>Assign Room</button>
            </div>
